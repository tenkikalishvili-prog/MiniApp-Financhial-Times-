...
class BudgetSub:
    ...
    # V2 (S16.1): несколько выплат в месяц — [{"day", "amount"}, …] или None.
    income_schedule: list | None = None
...
async def budget_overview(
    ...
    for cid, group, name, emoji, spent_raw, limit_raw, exp_day, exp_amount, inc_sched in result.all():
        ...
        view.subcategories.append(
            BudgetSub(
                ...
                income_schedule=inc_sched if isinstance(inc_sched, list) else None,
            )
        )
        view.spent += spent
        view.limit += limit
